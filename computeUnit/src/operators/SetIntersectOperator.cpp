...
WorkResponse SetIntersectOperator::run() {
    ...
    if (!innerCol) {
        response.set_info("[SetIntersectOperator] ERROR - Could not find the column (" + setData.innercolumn().tabname() + "." + setData.innercolumn().colname() + "). Aborting execution.");
        response.set_success(false);
        LOG_ERROR("[SetIntersectOperator] Could not find the column (" << setData.innercolumn().tabname() << "." << setData.innercolumn().colname() << "). Aborting execution." << std::endl;)
        return response;
    }

    if (!outerCol) {
        response.set_info("[SetIntersectOperator] ERROR - Could not find the column (" + setData.outercolumn().tabname() + "." + setData.outercolumn().colname() + "). Aborting execution.");
        response.set_success(false);
        LOG_ERROR("[SetIntersectOperator] Could not find the column (" << setData.outercolumn().tabname() << "." << setData.outercolumn().colname() << "). Aborting execution." << std::endl;)
        return response;
    }
    ...
    switch (innerCol->datatype) {
        ...
        case DataType::position_list: {
            rowCount = do_intersect_pl(innerCol, outerCol);
        } break;
        default: {
            response.set_info("[SetIntersectOperator] ERROR - Unsupported column type. Intersect is currently only implemented with bitmasks.");
            response.set_success(false);
            LOG_ERROR("[SetIntersectOperator] Unsupported column type. Intersect is currently only implemented with bitmasks." << std::endl;)
        }
    }
